...
const Song = (props) => {
    ...
    useEffect(() => {
        setFavourite(props.stared_user.some((user) => user.id === userId))
    }, [props])
        ...
        <div className={s.playlist__item}>
            <div className={s.playlist__track}>
                <div className={s.track__title}>
                    ...
                    <div>
                        <a className={s.track__titlelink} href="http://">{props.title}
                            <span className={s.track__titlespan}></span>
                        </a>
                    </div>
                </div>
                    <div className={s.track__author}>
                        <a className={s.track__authorlink} href="http://">{props.author}</a>
                </div>
                <div className={s.track__album}>
                    <a className={s.track__albumlink} href="http://">{props.album}</a>
                </div>
                <div>
                    <svg className={s.track__timesvg} alt="time" onClick={handleSetLike}>
                        <use xlinkHref={`${sprite}#icon-${isFavourite ? 'like' : 'dislike'}`}></use>
                    </svg>
                    <span className={s.track__timetext}>{(props.time / 60).toFixed(2)}</span>
                </div>
            </div>
        </div>
    );
